 ...
 */
export function wireRealtimeBoardActions(engine, getSessionId, getView, root = typeof document !== 'undefined' ? document : null) {
  if (!root) return () => {};
  ...
  async function handleClick(event) {
    ...
    if (button.dataset.realtimeAction === 'advance-round') {
      await mutateWithConflictRecovery(engine, sessionId, () => bridge.setRound(sessionId, (Number(view.roundNumber) || 0) + 1));
      return;
    }
    ...
    if (button.dataset.realtimeAction === 'toggle-token-visible') {
      ...
      if (!token) return;
      await mutateWithConflictRecovery(engine, sessionId, () => bridge.setTokenPublicState(sessionId, {
    ...
    if (button.dataset.realtimeAction === 'clear-initiative') {
      await mutateWithConflictRecovery(engine, sessionId, () => bridge.setInitiative(sessionId, []));
      return;
    }
    ...
    if (button.dataset.realtimeAction === 'adjust-own-hp') {
      ...
      await mutateWithConflictRecovery(engine, sessionId, () => bridge.updateCharacter(sessionId, {
